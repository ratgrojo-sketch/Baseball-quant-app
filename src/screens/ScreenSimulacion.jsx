...
function PitcherBlock({ label, teamRoster, data, onChange }) {
  ...
  return (
    <Collapsible
      title={label}
      sub={data.name ? `${data.name} · ERA ${data.era || "--"}` : "Sin selección"}
      defaultOpen
    >
      ...
      {data.era && (
        <div style={{ fontSize: 10, color: T.green, fontWeight: 700, marginBottom: 6,
          ...
          {Ico.check(T.green)} STATS CARGADAS · Editables
        </div>
      )}
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {HANDS.map((h) => (
            <button key={h} onClick={() => set("hand", h)} style={{
...
function LineupBlock({ label, teamRoster, lineup, onChange }) {
  ...
  const handleSelect = (idx, player) => {
    const next = [...lineup];
    if (!player) {
      next[idx] = { name: "", pos: POSITIONS[idx] || "DH", hand: "R" };
    } else {
      next[idx] = {
        playerId: player.id,
        name:  player.fullName || player.name,
    ...
    <Collapsible title={label} sub={`${filled}/9 bateadores${batters.length ? ` · ${batters.length} en roster` : ""}`}>
      {batters.length > 0 && filled === 0 && (
        <button onClick={() => {
          ...
          ⚡ Auto-fill lineup ({batters.length} jugadores)
        </button>
      )}
      {lineup.map((p, i) => (
    ...
    <Collapsible title={label} sub={`ERA ${data.era || avgEra} · Fatiga ${data.fatigue ?? 0}/3`}>
      {relievers.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          ...
          <button onClick={() => set("era", avgEra)} style={{
            marginTop: 8, background: T.raised, border: `1px solid ${T.border}`,
      ...
      <div style={{ marginTop: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          {[0, 1, 2, 3].map((f) => (
            <button key={f} onClick={() => set("fatigue", f)} style={{
...
export default function ScreenSimulacion({ selectedGame, onSimulate }) {
  ...
  useEffect(() => {
    ...
    if (hBP.length && !homeBP.era) setHomeBP((p) => ({
      ...p, era: (hBP.reduce((s, r) => s + parseFloat(r.era), 0) / hBP.length).toFixed(2),
    }));
    if (aBP.length && !awayBP.era) setAwayBP((p) => ({
      ...p, era: (aBP.reduce((s, r) => s + parseFloat(r.era), 0) / aBP.length).toFixed(2),
    }));
  }, [roster]);
